...
**Amended 2026-09-25.** This rule was written against a three-screen wizard and
the code had drifted to match a different three-screen wizard. It is now two
screens, and the rule is the reason:

**Flow:**

1. Splash screen: Logo + "NUMI", non-blocking, max 1.5 seconds, hidden as soon
   as Inter is measured. It never blocks the app shell.
2. Screen one: wallet name, wallet type, starting balance, **and** the
   light/dark choice. All four on one screen, because splitting them is what
   turns a setup into a wizard.
3. Screen two: the budgeting period, with editable start and end dates.
4. Screen three: assign money to categories. Every category is skippable and the
   totals-to-zero path is the fastest one, so this screen is a shortcut out of
   onboarding rather than a toll.
5. Home. `ProgressDots` shows position on all three.

A wallet is created on screen one and a period on screen two, so a user who
abandons at screen three still has a working home screen with an empty plan. 5. If both exist: Show Safe-to-Spend.
