...
## PillToggle (replaces SegmentedControl)
...
**Rule:** One CenterDockButton per screen. Never duplicate the primary action elsewhere on the same screen.

**Amended 2026-09-25.** This was `FAB`, bottom-right at 24px. It is now docked
centre-bottom, floating above the tab bar, at 66px. A centre dock reads as
"the thing you do here" and keeps the thumb on the same target on every tab.
Screens that need secondary actions use `CircleActionButton` instead.
...
| ------------------ | --- | -------------------------- | ----------------------------------------- |
| SafeToSpendHero    | Yes | `safe-to-spend-hero.tsx`   | Gauge arc, core of the product            |
| Button             | Yes | `button.tsx`               | 4 variants, 3 sizes                       |
| AmountInput        | Yes | `amount-input.tsx`         | Always-visible keypad, no OS keyboard     |
| TextField          | Yes | `text-field.tsx`           | Single and multiline                      |
| Card               | Yes | `card.tsx`                 | 4 tones; 1px border is load-bearing       |
| ListItem           | Yes | `transaction-row.tsx`      | Sign from `kind`, never from the string   |
| EmptyState         | Yes | `empty-state.tsx`          | 4 variants                                |
| BottomSheet        | Yes | `bottom-sheet.tsx`         | Mobile: sheet, Web: modal                 |
| Toast              | Yes | `app-toast.tsx`            | Success, Neutral, Warning. No error toast |
| PillToggle         | Yes | `pill-toggle.tsx`          | Replaces SegmentedControl for choice      |
| GroupRow           | Yes | `group-row.tsx`            | iOS Settings grouped rows                 |
| GemBadge           | Yes | `gem-badge.tsx`            | Faceted status tag                        |
| CircleActionButton | Yes | `circle-action-button.tsx` | Secondary actions                         |
